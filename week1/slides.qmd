...
`\vspace*{0.cm}\fontsize{13}{14}\sf`{=latex}
...
\begin{alertblock}{Lecturer: Professor Anastasios Panagiotelis}
\href{mailto:anastasios.panagiotelis@monash.edu}{\faicon{envelope}  anastasios.panagiotelis@monash.edu}

\href{https://anastasiospanagiotelis.netlify.app/}{\faicon{home} anastasiospanagiotelis.netlify.app}
\end{alertblock}

\begin{block}{Tutors}
\begin{itemize}\itemsep=0cm\parskip=0cm
\item \textbf{Lachlan Macquarie}
\item Sam Ferraro
...
`\placefig{10.2}{1.4}{width=5.8cm}{tableware2}`{=latex}
...
- The resale values are currently estimated by a group of specialists.
  They see me as a threat and do not cooperate.

## CASE STUDY 4: Airline

`\fullheight{ansettlogo}`{=latex}
...
- One hour in-person workshop each Tuesday
- One hour in-person tutorial on Wednesday or Thursday
...
ass <- bind_rows(
  ...
  tibble(
    Task = "Weekly Quizzes",
    `Due Date` = "in workshop, weeks 2--11",
    Value = "8%"
  ),
  ass[-1, ]
)
...
`\fontsize{18}{24}\sf\vspace*{0.3cm}`{=latex}
...
`\vspace*{-0.5cm}`{=latex}
`\centerline{\textbf{Please don't send emails. Use the forum.}}`{=latex}

## International Institute of Forecasters

`\placefig{1}{3}{width=4cm}{iifLOGO2}`{=latex}
...
Available for download from Posit:
...
`\placefig{4.2}{1.4}{width=3.8cm}{tsibble.png}`{=latex}
`\placefig{8.0}{1.4}{width=3.8cm}{tsibbledata.png}`{=latex}
`\placefig{2.3}{4.65}{width=3.8cm}{tidyverse.png}`{=latex}
`\placefig{6.1}{4.65}{width=3.8cm}{feasts.png}`{=latex}
`\placefig{9.9}{4.65}{width=3.8cm}{fable.png}`{=latex}
...
`\small\vspace*{-0.2cm}`{=latex}

1. The 7-day moving average of transit trade volumes (in millions of metric tons) through the Strait of Hormuz on 7 Aug 2026.
2. The difference in runs scored between Australia and Bangladesh in the second cricket test to be held 22--26 Aug 2026, Mackay.
3. Maximum temperature in Celsius at Melbourne airport on 13 Sep 2026
4. Google closing stock price in $US on 5 Oct 2026
5. The seasonally adjusted estimate of the number of employed people (in millions) for Sep 2026.
   ABS CAT 6202.

```{=latex}
\begin{block}{Due Monday 3 August}
...
```{=latex}
\only<3->{\begin{textblock}{1.13}(1.67,3.21)
\begin{alertblock}{}\fontsize{9}{9}\sf Index\phantom{dg}\end{alertblock}
\end{textblock}}
\only<4->{\begin{textblock}{4.2}(3.2,3.21)
\begin{alertblock}{}\fontsize{9}{9}\sf Keys\phantom{dg}\end{alertblock}
\end{textblock}}
\only<5>{\begin{textblock}{1.3}(7.8,3.21)
