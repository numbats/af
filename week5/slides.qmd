...
knitr::opts_chunk$set(dev.args = list(pointsize = 11, bg = "transparent"))
```
...
- We assume that the residuals are white noise (uncorrelated, mean zero, constant variance).
...
where
$$
  q^2_{T + h} = \frac{\displaystyle e^2_{T + h}} {\displaystyle\frac{1}{T - m}\sum_{t = m + 1}^T (y_{t} - y_{t - m})^2},
$$
...
tscv_plot <- function(.init, .step, h = 1) {
  expand.grid(
    ...
    scale_y_reverse() +
    scale_color_manual(
      values = c(train = "#0072B2", test = "#D55E00", unused = "gray")
    ) +
    guides(col = FALSE) +
    labs(x = "time", y = "") +
    theme_void() +
